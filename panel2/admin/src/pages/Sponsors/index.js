    ...
    <TableHead>
      <TableRow>
        {headCells.map((headCell) => (
          <TableCell key={headCell.id} style={{ fontWeight: 700 }} sortDirection={orderBy === headCell.id ? order : false}>
            <TableSortLabel
  ...
  return (
    <div>
      <Paper>
          ...
          <Table size='medium'>
            ...
            <TableBody>
              {stableSort(rows, getComparator(order, orderBy))
                ...
                .map(({ id, country, user, post, date, views, budget, expire, status }) => (
                  <TableRow hover tabIndex={-1} key={id}>
                    ...
                    <TableCell
                      onClick={() => push(`${url}/${id}`)}
                      style={{ textDecoration: 'underline', color: '#b910b9', textTransform: 'uppercase', cursor: 'pointer' }}
                    >
                      {status}
                    </TableCell>
                  </TableRow>
        ...
        <TablePagination component='div' count={rows.length} rowsPerPage={rowsPerPage} page={page} onChangePage={handleChangePage} />
      </Paper>
